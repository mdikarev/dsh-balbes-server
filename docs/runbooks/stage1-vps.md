...
Дальше: Этап 2 (сервер + админка) — `docs/runbooks/stage2-vps.md`.

> Применимость: этот runbook фиксирует **Этап 1** (headless-инструмент). На
> текущей `main` профиль `balbes` — уже сервер-демон Этапа 2 (host + админка,
> headless удалён): для развёртывания и проверок используйте runbook Этапа 2.
> Ниже Этап 1 описан как есть — для истории и контекста; smoke-проверка Этапа
> 1 заменена проверкой через админку/curl (кнопка тестового промпта) из
> runbook'а Этапа 2.
...
5. **Ключ.** Если `DEEPSEEK_API_KEY` не задан в окружении — запрашивает ключ
   ...
   `$DSH_HOME/.credentials.yaml` (права `600`). Если файла ещё нет — он
   создаётся целиком (`version: 1`, `refs:`, `records: {}`). Если файл уже
   существует, чужие секции никогда не перезаписываются: при наличии чистой
   строки `refs:` ключ дописывается в неё одной строкой; в остальных случаях
   (секции `refs:` нет, она задана строкой вида `refs: {...}`, либо ключ
   `DEEPSEEK_API_KEY` уже есть в файле) скрипт файл не трогает —
   предупреждает и показывает формат для ручной правки.
...
*(Этап 1.)* Установщик Этапа 1 **не запускает LLM автоматически** (продуктовое
решение): smoke Этапа 1 выполняется вручную одной командой:
...
Начиная с Этапа 2 профиль — сервер-демон, и эта команда больше не является
smoke-проверкой: актуальная проверка — логин в админку (`http://<IP>:8080`) и
кнопка тестового промпта, либо curl + JWT (раздел «Smoke без браузера» в
...
(`run_priv npm i -g`; sudo нужен только на расхождении) и удаляет устаревшее
зеркало `$DSH_HOME/profiles/node_modules/@deepseek-ai`, после чего продолжает
установку. Версию в тексте не хардкодьте: она живёт только в
`scripts/engine-version.txt`. Повторный запуск установщика (в т.ч.
`curl ... | bash`) = обновление репозитория, движка и сервиса. Обновить движок
вручную (например, без установщика):
...
существовал, установщик чужие записи не трогает: в чистую секцию `refs:` ключ
дописывается автоматически; если это невозможно (нет `refs:`, она задана
строкой вида `refs: {...}`, либо `DEEPSEEK_API_KEY` уже есть в файле) — файл не
перезаписывается, и ключ нужно добавить вручную в секцию `refs`:
