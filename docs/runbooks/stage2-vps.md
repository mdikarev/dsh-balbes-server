...
После обновления откройте админку и войдите обычными учётными данными.
В сайдбаре выберите **«Telegram»**:
...
3. Вставьте token и User ID в форму; token после сохранения отображается только
   как `••••`.
4. Нажмите **«Проверить подключение»** — ожидается имя бота и успешный статус.
5. Включите интеграцию и сохраните настройки.
6. В личном чате с ботом отправьте `/start`, выберите «Воркспейсы», затем
   `Дом агента` или проект.
7. Отправьте текстовую задачу; ожидается сообщение «Задача принята» и затем
   финальный ответ штатного dsh agent loop. Через «Файлы» проверьте навигацию
   по дереву и просмотр текстового файла.

Если token удалить, polling автоматически останавливается. Если интеграцию
выключить, token сохраняется, но Telegram update не обрабатываются. Входящие
сообщения из групп и от другого User ID должны игнорироваться без раскрытия
списка воркспейсов.
