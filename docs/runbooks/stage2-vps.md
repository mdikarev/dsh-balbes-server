...
произвольную страницу по URL (`web_fetch` — это канал отправки данных наружу)
...
Если ключа нет, задача не сломается: вызов `web_search` завершится ошибкой
инструмента (код `WEB_PROVIDER_CREDENTIAL_MISSING`, в тексте — «no API key»),
остальные инструменты работают, а агент сообщит об ошибке поиска. Загрузка
произвольной страницы по URL при этом недоступна намеренно: `web_fetch` — канал
отправки данных наружу (по ссылке из подсунутого текста можно отправить
содержимое воркспейса на чужой адрес), а поиск адресуется провайдеру, а не
произвольному серверу.
...
4. Попросите открыть конкретный URL (например: «открой
   ...
   канал отправки данных наружу, и его отсутствие и есть проверка границы
   (поиск из пункта 3 при этом работает).
