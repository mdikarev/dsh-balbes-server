...
7. В личном чате с ботом отправьте `/start` — ожидается карточка-меню
   **«🤖 Агент сервера»**: строки состояния (`Воркспейс не выбран`, `Задача:
   нет активной задачи`; `Модель: …` — когда подключён раздел моделей) и
   кнопки **«📁 Воркспейсы»**, **«🧠 Модель»**, **«🔄 Обновить»**. Нажмите
   «📁 Воркспейсы» и выберите `Дом агента` или проект — ожидается
   `Выбран: …`, а кнопки сменятся на **«📄 Файлы»**, **«🧠 Модель»**,
   **«📁 Воркспейс»**, **«🔄 Сбросить контекст»**, **«⏹ Стоп»** и
   **«🔄 Обновить»**. Неизвестная команда (например `/foo`) отвечает
...
8. Отправьте текстовую задачу; ожидается живая карточка **«⏳ <воркспейс> ·
   время · шаг N»**, которая обновляется на месте, пока задача выполняется, и
   превращается в квитанцию **«✅ Готово · время · N шагов»**. Финальный ответ
   штатного dsh agent loop приходит отдельным сообщением (длинный ответ —
   несколькими сообщениями). Кнопка **«Файлы»** открывает дерево выбранного
   воркспейса:
...
`Модель: …`, когда подключён раздел моделей) и кнопками «📁 Воркспейсы»,
«🧠 Модель», «🔄 Обновить». «📁 Воркспейсы» открывает список: `Дом агента` и
проекты; выбор проекта → `Выбран: …` и полный набор кнопок («📄 Файлы»,
«🧠 Модель», «📁 Воркспейс», «🔄 Сбросить контекст», «⏹ Стоп»,
«🔄 Обновить»). Неизвестная команда (например `/foo`) → «Не знаю такой
команды.» и та же карточка; текст с `/` агенту не уходит.

**Шаг 5. Задача.** Текстовое сообщение → живая карточка **«⏳ <воркспейс> ·
время · шаг N»**, которая обновляется на месте, пока задача идёт, и становится
квитанцией **«✅ Готово · время · N шагов»**; финальный ответ приходит отдельным
сообщением (длинный — несколькими сообщениями). Проверьте, что агент действительно
