...
Логин и `jwtSecret` при сбросе не меняются, поэтому старый пароль сразу
перестаёт подходить (файл перечитывается при каждом входе), но **ранее
выданные JWT остаются действительными** до истечения (до 24 ч). Команда
требует уже выполненной установки: она читает существующий
`$DSH_HOME/admin-auth.json` и собранный host. После сброса войдите в админку
с новым паролем.
