...
7. В личном чате с ботом отправьте `/start` — ожидается карточка-меню
   ...
   **«⏹ Стоп»** и **«🔄 Обновить»**. Список команд бот регистрирует в Telegram
   ...
   присылает ту же справку текстом, `/menu`, `/start` и `/status` — ту же
   карточку. Неизвестная команда (например `/foo`) отвечает «Не знаю такой
   команды.» и той же карточкой; текст, начинающийся с `/`, агенту не уходит —
   его обрабатывает бот.
8. Отправьте текстовую задачу; ожидается живая карточка **«⏳ <воркспейс> ·
   ...
   **«✅ Готово · время · N шагов»**. Финальный ответ штатного dsh agent loop
...
полем — кнопка «Меню». `/menu`, `/start` и `/status` присылают эту же
карточку, `/help` — справку («/stop — остановить задачу (контекст
сохраняется)», «/reset — сбросить контекст (сессия удаляется)»).
«📁 Воркспейсы» открывает список: `Дом агента` и проекты; выбор строки
(`Дом агента` или проект) → **новое** сообщение `Выбран: …` с полным набором
кнопок («📄 Файлы», «🧠 Модель», «📁 Воркспейс», «🔄 Сбросить контекст»,
«⏹ Стоп», «🔄 Обновить»). Неизвестная команда (например `/foo`) → «Не знаю такой
команды.» и та же карточка; текст, **начинающийся** с `/`, агенту не уходит —
его обрабатывает бот (путь вроде `/etc/hosts` внутри текста задачи уходит
агенту как обычно).
...
подтверждение («Да, сбросить» / «Отмена») → «Контекст сессии сброшен»:
