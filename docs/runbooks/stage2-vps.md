...
2. Попросите прочитать файл вне воркспейса (например
   `$DSH_HOME/.credentials.yaml` или файл другого проекта) — ожидается
   содержимое файла: отдельного read-guard у канала нет, чтение доступно
   настолько, насколько позволяет ОС.
