...
- **Этап 1 выполнять не нужно**: установщик Этапа 2 сам ставит окружение
  (Node ≥ 22, pnpm, git), dsh и профиль. Ключ модели при установке больше не
  запрашивается — он задаётся в админке, в разделе «Модели». Если Этап 1 уже
  был выполнен на этой машине — повторный запуск установщика аккуратно обновит
  профиль до сервера (ключ в `$DSH_HOME/.credentials.yaml` при этом
  сохраняется).
...
Скрипт **не запрашивает ключ модели**: ключ задаётся после установки в
админке, в разделе «Модели» (закреплённая карточка DeepSeek → «Изменить
ключ»). Для автоматизированной установки ключ можно необязательно передать
заранее через переменную окружения (без `export` её увидит только curl, а не
bash, исполняющий скрипт):
...
Если `DEEPSEEK_API_KEY` задан, установщик молча запишет его в
`$DSH_HOME/.credentials.yaml` (права `600`); если нет — файл не трогается, а
ключ вы задаёте в админке.
...
8. **Ключ API (только из окружения).** Интерактивного запроса нет: если
   `DEEPSEEK_API_KEY` задан — ключ записывается в `$DSH_HOME/.credentials.yaml`
   (права `600`), по правилам Этапа 1 (существующий файл и чужие записи не
   перезаписываются); если не задан — шаг пропускается, а ключ задаётся в
   админке, в разделе «Модели».
...
- `DEEPSEEK_API_KEY` — необязательный ключ DeepSeek для неинтерактивной
  установки; если не задан, ключ задаётся в админке (раздел «Модели»);
...
На свежей установке ключ модели ещё не задан — установщик его больше не
запрашивает. Перед тестовым промптом откройте раздел **«Модели»**, у
закреплённой карточки DeepSeek нажмите **«Изменить ключ»**, вставьте ключ и
сохраните. Ключ применяется на лету (файл `.credentials.yaml` наблюдается
процессом), рестарт сервиса не нужен; после этого работает и кнопка
«Отправить тестовый промпт». Для неинтерактивной установки ключ можно
передать заранее через `DEEPSEEK_API_KEY` (см. «Установка одной командой»).
...
каталогу (`/api/workspaces/tree`). У нового проекта корень не пуст: при
создании в нём идемпотентно появляется каталог `.dsh/` (проектные скиллы);
«Каталог пуст» показывается только для действительно пустого каталога.

Клик по листу-файлу открывает его содержимое в динамическом табе правой
зоны (таб **«Сессии»** остаётся первым и незакрываемым). Файл показывается по
типу: markdown (`.md`/`.markdown`) — отрендеренным документом (заголовки,
списки, таблицы, ссылки, блоки кода) с переключателем **«Просмотр» /
«Исходник»** (по умолчанию «Просмотр»); код известного языка (`.ts`, `.py`,
`.json`, `Dockerfile`, …) — с подсветкой синтаксиса; неизвестный тип — как
раньше, моноширинным текстом. Большой файл — с пометкой «Файл показан не
полностью», пустой — «Файл пуст», бинарный и симлинк — понятное «нельзя
показать». Кнопка **«Обновить»** перечитывает активный таб.
...
7. В личном чате с ботом отправьте `/start` — ожидается карточка-меню
   ...
   **«🧠 Модель»**, **«🗂 Сессии»**, **«📁 Воркспейс»**,
   **«🔄 Сбросить контекст»**, **«⏹ Стоп»** и **«🔄 Обновить»**. Список команд
   бот регистрирует в Telegram сам: по «/» у поля ввода виден список `/menu`,
   `/status`, `/ws`, `/model`, `/sessions`, `/reset`, `/stop`, `/help`, а рядом
   с полем — кнопка **«Меню»**; `/help`
...
8. Отправьте текстовую задачу; ожидается живая карточка **«⏳ <воркспейс> ·
   ...
   **«🔄 Сбросить контекст»** (и команда `/reset`) начинает новую сессию
   воркспейса — следующая задача стартует с чистого контекста, а прежняя сессия
   **не удаляется**: она остаётся в реестре и доступна через **«🗂 Сессии»** /
   `/sessions`. Карточка «Сессии» показывает активный список воркспейса:
   строка `• <заголовок>` — активная сессия, нажатие заголовка возвращает к
   ней (штатный resume), кнопка **«🗄»** убирает сессию в архив, **«🗄 Архив»**
   открывает скрытые, **«↩»** возвращает сессию из архива, **«⬆ К сессиям»** —
   назад к активному списку. Архивация активной сессии отсоединяет её — как
   `/reset`; следующая задача создаст новую. **«⏹ Стоп»** (и команда `/stop`)
...
насколько позволяет ОС, запись вне writable-root ограничена sandbox. Гейтованная
операция приходит владельцу карточкой «🔐 Запрос подтверждения · <воркспейс>» с
кнопками «✅ Разрешить один раз» / «⛔ Отклонить»; если ответа нет 5 минут, запрос
разрешается отказом (fail-closed) — операция не выполняется, а не зависает.
...
# 3) Тестовый промпт с JWT (ключ DeepSeek задайте сначала в админке,
#    раздел «Модели»)
...
# корень проекта: у нового проекта каталог .dsh (проектные скиллы)
curl -sS -X POST http://127.0.0.1:8080/api/workspaces/tree \
  -H "authorization: Bearer $TOKEN" -H 'content-type: application/json' \
  -d '{"scope":"project","name":"alpha","path":""}' -w '\nHTTP %{http_code}\n'
#    ожидается: {"entries":[{"name":".dsh","kind":"dir"}]} и HTTP 200
#    (dot-записи включены; пустой entries — только у действительно пустого
#     каталога)
...
видно `/menu`, `/status`, `/ws`, `/model`, `/sessions`, `/reset`, `/stop`,
`/help`, рядом с полем — кнопка «Меню». `/menu` и `/start` присылают эту же
карточку, `/status` — ту же карточку плюс строку `Сессия: активна` или
`Сессия: не создана`, `/help` — справку («/stop — остановить задачу
(контекст сохраняется)», «/reset — начать новую сессию (прежние остаются в
/sessions)», «/sessions — сессии воркспейса: список, возврат, архив»).
...
«🧠 Модель», «🗂 Сессии», «📁 Воркспейс», «🔄 Сбросить контекст», «⏹ Стоп»,
«🔄 Обновить»). Неизвестная команда (например `/foo`) → «Не знаю такой
команды.» и та же
...
5. Негативный контроль, что границы реальны: попросите записать файл за
   ...
   Ожидается карточка «🔐 Запрос подтверждения» с кнопками «✅ Разрешить один
   раз» / «⛔ Отклонить», а не тихая запись. Нажмите «⛔ Отклонить» (или не
   отвечайте 5 минут — запрос истечёт в отказ): файл не записывается, агент
   получает отказ. Границы задачи — это sandbox-политика процесса
   (`workspace-write` + `ask`), а решение по гейтованной операции принимает
   владелец.

**Шаг 7. Файлы, сессии и сброс контекста.** Кнопка «📄 Файлы» открывает дерево
...
карточке) → «Контекст сессии сброшен»: следующая задача начинается **новой**
сессией (агент не помнит прошлую), а прежняя **не удаляется**.

Smoke списка и архива (та же сессия чата): отправьте две задачи подряд и
дождитесь квитанций, затем `/reset` (подтвердите) и отправьте ещё одну задачу.
Откройте «🗂 Сессии» (или `/sessions`) → карточка `🗂 Сессии · <воркспейс>`:
в списке **две** сессии — прежняя (без `•`) и активная (помечена `• `, в неё
ушла задача после `/reset`). Возврат: нажмите заголовок прежней сессии →
«Выбрана сессия: …», активная отметка переезжает на неё, следующая задача идёт
в неё. Архив: нажмите **«🗄»** у любой сессии → строка исчезает из активного
списка; **«🗄 Архив»** открывает «🗄 Архив сессий · <воркспейс>», где она
лежит, а **«↩»** возвращает её в активный список; **«⬆ К сессиям»** — назад к
списку, **«⬅ Меню»** — к карточке. Архивация активной сессии отсоединяет её —
следующая задача создаёт новую, а архивную сессию можно вернуть нажатием
заголовка (она выходит из архива и становится активной).
...
Откройте админку, раздел **«Модели»**: у закреплённой карточки DeepSeek
нажмите **«Изменить ключ»**, вставьте ключ и сохраните. Ключ применяется на
лету (файл `$DSH_HOME/.credentials.yaml`, секция `refs`, имя
`DEEPSEEK_API_KEY`, права `600`, наблюдается процессом) — рестарт не нужен:

```bash
curl -fsS -X POST http://127.0.0.1:8080/api/prompt -H "authorization: Bearer $TOKEN" -d '{"prompt":"Напиши ok"}'
```

Если `DEEPSEEK_API_KEY` задан в окружении запуска (юнит или shell), запись из
админки будет теневой — уберите переменную из окружения сервиса; установщик в
юнит её не прописывает.
...
# ожидается: {"ok":true,"result":[{"command":"menu",…}, …]} — восемь команд
```
...
- `telegram-state.json` — состояние Telegram-канала (права `600`, пишется
  ...
  агента по воркспейсам и архив (`archived`). Форма файла (`version: 1`,
  `activeWorkspace`, `sessions`, `archived`, `offset`) совместима с файлом
  без `archived` — его чат читает как пустой архив; выбранной в Telegram
  модели здесь нет — она живёт в секции
