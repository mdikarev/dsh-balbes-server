...
агент сообщит об ошибке поиска. Если же ключа нет **вообще**, дело не в поиске:
без ключа не проходит сам вызов модели, поэтому задача падает раньше поиска и
ровно так же, как в разделе «Нет ключа модели» (ошибка агента, а не отчёт о
поиске) — сначала восстановите ключ, потом проверяйте поиск.
...
3. Попросите найти что-то в интернете (например: «найди в интернете, какая
   ...
   `WEB_PROVIDER_CREDENTIAL_MISSING`) — это отсутствующий ключ модели, а не
   поломка канала (см. «Модели» и «Нет ключа модели»).
