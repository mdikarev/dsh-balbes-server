...
5. **Профиль.** Синхронизирует `profiles/balbes` из репозитория в
   ...
   каждое обновление стирало бы все настройки.
...
- `telegram-state.json` — состояние Telegram-канала (права `600`, пишется
  ...
  модели здесь нет — она живёт в записи
  `agent-default-model` профильного `cordis.patch.yml`. Токен бота здесь **не
...
- `profiles/balbes/cordis.patch.yml` — **настройки dsh на 0.1.7**: базовый
  ...
  модель по умолчанию (`agent-default-model`). При обновлении установщик
  сохраняет эти записи (шаг 5 установки); вручную их не редактируйте — пишите
