...
и служебные пометки: чтение файлов и изображений (`read`, `read_image`), запись
и правка файлов (`write`, `edit`), поиск по файлам (`glob`, `grep`), список дел
(`todo_write`) и цели (`get_goal`, `create_goal`, `update_goal`) — и ничего
больше. Поэтому из Telegram-задачи **нельзя** выполнить команду оболочки
(`bash`, `pwsh`), открыть файл через `str_replace_editor`, подключить навык
(`skill`), делегировать работу субагентам (`subagent`, `subagent_fork`,
`workflow`, `ralph`), обратиться в сеть/интернет (`web_fetch`, `web_search`) или
управлять заданиями и агентами (`job_list`, `job_output`, `job_kill`,
...
навыки, субагенты или сеть — используйте другую поверхность сервера:
...
**Шаг 6. Границы канала.** Две проверки:
1. Попросите задачу, требующую оболочки (например: «выполни `whoami` через
   ...
   это норма: у Telegram-задачи только файловые инструменты воркспейса и
   планирование (см. «Настройка Telegram через админку», абзац про
   ограниченный набор).
...
Оболочка (`bash`), навыки, субагенты и сеть в Telegram-задаче **недоступны по
дизайну** — канал ограничен файлами выбранного воркспейса и служебными
