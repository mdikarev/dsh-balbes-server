...
- Telegram: владелец в админке сохраняет token и Telegram user ID через
  ...
  Границы канала: агентская задача получает только файловые инструменты воркспейса, поиск в интернете и инструменты планирования (allow-фильтр per-agent поверхности инструментов),
  поэтому shell, `str_replace_editor`, skill, subagent/workflow и загрузка произвольного URL (`web_fetch`) недоступны — задача из одного проекта не может прочитать другой проект
  или `$DSH_HOME`. Поиск (`web_search`) идёт через провайдера DeepSeek тем же ключом, что и раздел «Модели»; `web_fetch` закрыт как канал отправки данных наружу.
  Чтение относительных и абсолютных путей дополнительно проходит containment-guard в scope агента; записи ограничены sandbox-политикой движка.
