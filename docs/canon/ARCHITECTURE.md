...
- Воркспейсы: при старте сервера дом `$DSH_HOME/agent/` обеспечивается
  ...
  Владелец правит файлы воркспейсов напрямую — это обычные файлы.
...
- Не входит (следующие этапы): git (импорт/клонирование, ключи и токены),
  ...
  доставка (SSE/WS), каналы (telegram/A2A), память (Qdrant), самообучение,
  мультиюзерность, замена драйвера цикла, граф-конфиг.
