...
- `packages/plugins/dsh-balbes-workspaces/` — плагин воркспейсов: сервис
  ...
  `$DSH_HOME/projects.json`. Подключается insert-записью в патч профиля;
...
- Воркспейсы: при старте сервера дом `$DSH_HOME/agent/` обеспечивается
  ...
  файлы `AGENTS.md`, `self.md` и каталог `skills/` с `README.md` — только
  ...
  `create` создаёт пустой каталог проекта (конфликт с существующим → 409) и
  upsert строки реестра; `delete` рекурсивно удаляет каталог и чистит строку.
