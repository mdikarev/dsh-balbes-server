...
- `packages/plugins/dsh-balbes-sessions/` — функциональный плагин сессий
  ...
  (`sessionQuery.readSession`). Источник транскрипта гибридный: реплики
  владельца/модели/инструментов — durable append-origin события лога
  (замещения и компакция не стирают уже увиденное); системный промпт и
  служебный контекст — только текущая модельная поверхность (`foldSurface`);
  флаг `inContext` у каждой строки показывает, входит ли событие в текущую
  поверхность. Нормализация — на сервере (`deriveEventMessage`), SPA рендерит.
  Писатели —
...
- Просмотр диалога сессии в админке: клик по строке таба «Сессии» открывает
  ...
  из durable append-origin событий, системный промпт и служебный контекст — из
