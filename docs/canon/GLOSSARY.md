...
закреплён, URL фиксирован, только ключ) или `custom` (OpenAI-совместимый),
имени, опционального `baseURL` и одного API-ключ-реф
в `$DSH_HOME/.credentials.yaml`. Значение ключа API не возвращает — только
признак `hasKey`; запись/сброс ключа идут через сервис `ctx.credentials`.
