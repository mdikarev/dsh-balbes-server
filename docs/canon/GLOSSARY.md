...
Token хранится через `ctx.credentials`, user ID и enabled — в settings;
настройка выполняется из админки. Канал не является отдельным процессом и не
соединяется с телефоном напрямую.
